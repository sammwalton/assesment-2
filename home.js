...
let canWeDeliver = (zipcode) => {
    ...
    }else{ return "Sorry, we don't deliver to that address"}
}
...
// CODE HERE
let canWeDeliverTwo = (zipecode2) => {
    for(let i = 0; i < deliveryAreaZipCodes.length; i++){
        if(deliveryAreaZipCodes[i] === zipecode2){
            return "we can deliver"
        } else {return "we can't deliver"
    }
}
}
...
//CODE HERE
const newMonth = deals[1].desc.replace('March', 'April').trim()
